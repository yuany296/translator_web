...
- 全局术语库：
  ...
  - 默认用本地 Kiwi 从每张图片的最终 OCR 块中发现韩文/英文专名，并按当前章节汇总到“待确认”页签
  - 自动发现不调用翻译 AI；本地服务离线时只跳过发现，不影响 OCR 和翻译
- 覆写渲染：
  - 按百分比坐标绝对定位覆写层
...
├─ term-discovery-core.js  # 待确认术语、去重、忽略与存储上限
...
1. 按 `local-ocr-service/README.md` 安装并启动本地服务；即使 OCR 使用百度模式，术语发现仍需要该服务在线。
2. Popup 中“自动发现新术语”默认开启，并显示 Kiwi 在线状态和待确认数量。
3. 每张图翻译完成后，扩展只把最终 OCR 原文块发送给本地 Kiwi。人名、作品名、英文组合名等候选会按去除 hash、保留 query 的页面 URL 聚合成一话。
4. 在术语管理页切到“待确认”，可以编辑固定译文并单条确认，或用“加入所有已填写”批量加入。还可以仅在本话忽略、永久忽略及恢复永久忽略项。
5. 候选正好占满一个 OCR 块且译文唯一时会预填建议；多个上下文出现冲突译文时会留空并标记“可能有歧义”。全名与无姓称呼（如 `김성현`、`성현`）是两个独立候选。

待确认区保留最近 20 话、每话最多 200 个候选，每个候选最多展示 3 组上下文。第一版只自动发现韩文和英文专名，不处理日文专名。
