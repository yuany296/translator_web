...
function getEnabledEntries(value) {
  return normalizeGlossary(value).entries.filter(entry => entry.enabled).sort((left, right) => right.source.length - left.source.length || left.source.localeCompare(right.source));
}
