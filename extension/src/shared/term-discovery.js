...
function normalizePendingStore(value) {
  const chapters = (value && Array.isArray(value.chapters) ? value.chapters : []).map(normalizeChapter).filter(Boolean).sort((left, right) => right.updatedAt - left.updatedAt).slice(0, MAX_CHAPTERS);
  return {
...
function normalizeChapter(value) {
  ...
  const candidates = (Array.isArray(value.candidates) ? value.candidates : []).map(normalizePendingCandidate).filter(Boolean).slice(0, MAX_CANDIDATES_PER_CHAPTER);
  return {
...
function normalizePendingCandidate(value) {
  ...
  const suggestedTargets = uniqueStrings(value.suggestedTargets, 5).map(item => String(item).trim().slice(0, 120)).filter(Boolean);
  return {
    source,
    ...
    contexts: (Array.isArray(value.contexts) ? value.contexts : []).map(normalizeContext).filter(Boolean).slice(0, MAX_CONTEXTS_PER_CANDIDATE),
    suggestedTargets,
...
function normalizeIgnoredStore(value) {
  const sources = (value && Array.isArray(value.sources) ? value.sources : []).map(item => {
...
function normalizeBlocks(value, targetKey = "") {
  return (Array.isArray(value) ? value : []).map((block, index) => {
...
function mergeDiscoveryResult({
  ...
  const glossaryKeys = new Set((glossary && Array.isArray(glossary.entries) ? glossary.entries : []).map(entry => getSourceKey(entry && entry.source)).filter(Boolean));
  ...
  for (const rawCandidate of Array.isArray(extractedCandidates) ? extractedCandidates : []) {
    ...
    for (const evidenceId of uniqueStrings(rawCandidate.evidenceIds, MAX_EVIDENCE_IDS_PER_CANDIDATE)) {
      ...
      if (getSourceKey(block.originalText) === sourceKey && block.translatedText && getSourceKey(block.translatedText) !== sourceKey && !candidate.suggestedTargets.includes(block.translatedText)) {
  ...
  chapter.processedEvidenceIds = uniqueStrings([...chapter.processedEvidenceIds, ...blocks.map(block => block.id)], MAX_PROCESSED_IDS_PER_CHAPTER);
  chapter.candidates = chapter.candidates.filter(candidate => !glossaryKeys.has(candidate.sourceKey)).sort((left, right) => right.score - left.score || right.updatedAt - left.updatedAt).slice(0, MAX_CANDIDATES_PER_CHAPTER);
  store.updatedAt = now;
  store.chapters = store.chapters.sort((left, right) => right.updatedAt - left.updatedAt).slice(0, MAX_CHAPTERS);
...
function getSuggestedTargetForSource(sourceValue, contextsValue) {
  ...
  const targets = uniqueStrings((Array.isArray(contextsValue) ? contextsValue : []).filter(context => getSourceKey(context && context.originalText) === sourceKey).map(context => String(context && context.translatedText || "").trim()).filter(target => target && getSourceKey(target) !== sourceKey), 5);
