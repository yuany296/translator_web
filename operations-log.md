...
## 2026-08-09 - Codex（修复页面重复标注被合并为一个译文框）

- 使用 Edge 连接到用户当前 Kakao 阅读页复查 DOM，确认旧的重复译文折叠修复后两个页内结果已分开，但仍残留一个覆盖上下两页的大型 `mt-cross-page-overlay`。本地服务保存的现场接缝 OCR JSON 给出了直接证据：文字框 `(341, 116, 329×43)` 完全位于接缝画布的下半页，而自动识别的 `caption_panel` 背景框 `(248, 73, 512×119)` 跨过了第 96 像素的页缝；旧逻辑把“背景面板跨页”误当成“文字跨页”，因而生成错误 canonical。
- 接缝筛选现在只允许可靠的 `speech_bubble` 视觉边界补充文字框的跨页证据；`caption_panel` 的背景延伸不能把单页文字升级为跨页文字。文字框自身真正穿过页缝时仍按原逻辑保留。OCR 几何版本升级到 `detect-crop-recognize-appearance-layout-v5`，使旧的错误接缝缓存自动失效。
- 同时保留第一轮通用保护：完整相同译文只有在原文相关，且共享 OCR 检测身份或二维文字框实际重叠至少 8% 时才折叠；真实重复标注保留独立结果。新增回归覆盖分离重复标注、重叠重复读数，以及“caption panel 背景跨页但文字只在下页”的现场结构；真正跨页 speech bubble 的既有正向用例继续通过。
- 构建成功；接缝/重复标注定向测试 `43/43`、完整 Node `845/845`、Python `86 passed, 1 skipped` 通过。仓库完整行数门禁仍被本次修改之外的既有超长文件阻塞：`scene-index-builder.js` 504 行、`capture-payload.js` 439 行、`renderer-crosspage.js` 429 行；全量 ESLint 仅对前两者报告既有 `max-lines`。
- Edge 扩展控制通道已能读取现场页，但浏览器安全策略禁止自动打开 `edge://extensions/`，Microsoft Edge 桌面控制入口又受本机 Codex 安装目录权限阻塞。因此最新 `dist/extension` 需要用户在扩展管理页手动点一次“重新加载”，再刷新当前 Kakao 页完成最终现场复核。

## 2026-08-09 - Codex（限制漫画背景覆盖并修复相邻译文遮挡）

- 现场调试数据确认两个问题来自统一渲染链路：背景覆盖使用了可能大于最终 canonical 蓝框的 `fill_box`，并继承 OCR 文字旋转角度，旋转后的矩形四角会越出蓝框；同时每个区域按“背景、译文”交错插入 DOM，后绘制的大标题背景可能压住先绘制的作者署名译文。
- 背景覆盖几何现统一取 `fill_box` 与最终蓝框的交集。旋转角度、polygon 与倾斜填充完全沿用 Git `HEAD` 的既有逻辑，不再另行改写；仅在外面增加一个不旋转、位于最终蓝框内的裁剪节点，使既有旋转效果保留而可见背景无法越界。该约束位于通用气泡渲染入口，不依赖当前截图或特定文字。
- 覆盖层改为先插入全部背景、再插入全部译文，保证任何相邻区域的背景都不能凭 DOM 顺序盖住另一个文字节点。新增回归覆盖超大背景裁剪、背景/文字旋转分离及全局层级顺序，并更新旧的渲染契约测试。
- 页面复核指出裁剪后仍有背景越界；代码追踪确认倾斜纯色区域的译文节点仍会额外创建 `.mt-fill-tilted`，这份背景位于外层裁剪节点之外。译文专用节点现在不再创建任何倾斜/水平填充，CSS 同时显式隐藏遗留的倾斜填充子节点；背景只允许从受蓝框裁剪的 cover 层显示，原有旋转参数保持不变。
- 新截图进一步确认蓝色最终框本身是旋转矩形，而第一版外层裁剪仍沿页面轴放置，cover 内的 `.mt-fill-tilted` 还会反向抵消旋转。裁剪节点现与文字使用相同规范化角度，内部背景只做局部居中、不再二次旋转或反向抵消；旋转与裁剪由同一个外框完成，使背景视觉角度和蓝框一致且不能越界。
- 第二张现场图暴露出旋转中心仍不一致：外层曾采用 `fill_box` 与蓝框的交集作为裁剪几何，交集框绕自身中心旋转后可能越出绕另一中心旋转的蓝框。外层现始终使用 canonical 最终蓝框及其中心，交集只决定内部背景的位置；因此所有可见背景最终都由蓝框本身裁断。
- 定向测试 `46/46`、完整 Node `848/848`、Python `86 passed, 1 skipped` 通过；修改源文件 ESLint、扩展构建与 `git diff --check` 通过。修改文件均满足现有行数限制，完整仓库仍只有既有三个超长文件阻塞全局行数门禁。

## 2026-08-09 - Codex（倾斜译文背景严格限制在蓝框内）

- Chrome 现场 DOM 对比确认，`[仅对金率音可见]` 的蓝色 OCR 框对应旋转后的排版框 `x=19.2, y=9.5, w=61.35, h=6.94`；此前背景裁剪误用了多个倾斜文字片段合并后的 polygon AABB，范围扩大为 `y=5.55～20.39`，所以长框上下越界尤其明显。其他倾斜框也存在同类、但幅度更小的扩大。
- 通用覆盖层现以未扩张的排版框作为裁剪矩形，并和文字使用同一个完整 OCR 倾角；清理图在框内只按页面坐标取样，不再二次旋转。译文排版、背景外轮廓和调试标注继续保持一致倾角，背景可见范围则始终由旋转后的蓝框裁断。
- 新增现场几何回归，确保扩张 polygon AABB 不能扩大背景覆盖；倾角、覆盖层和样式定向测试 `70/70` 通过，扩展已重新构建到 `dist/extension/`。最终 Chrome 现场复核需在扩展管理页重新加载本次构建后执行。
- 首次重载后的四角复核发现，旋转排版框仍比蓝色 OCR polygon 横向大约 `1.3%`（约 9px）。上游根因是 `mergePercentPolygons()` 对单个最终 OCR 框也再次拟合，并在非 1:1 图片上把物理角度错误应用到百分比坐标，导致法线方向膨胀。现对单框直接保留原始四点，多框才重新求并集；坐标模型版本升级为 `crop-source-transform-v3`，使旧的膨胀 OCR 缓存失效。
- 渲染层最终使用原始 polygon 的 AABB 承载页面坐标背景，并以 polygon 本身裁剪；背景图不旋转，倾角由多边形边界表达，文字与调试标注继续使用同一 OCR 角度。上游合并、内容渲染和样式联合回归 `88/88`、修改文件 ESLint、扩展构建及 `git diff --check` 通过。
- 最终 Chrome 现场复核：目标 `t0` 背景 polygon 与蓝框四点最大偏差 `0.00497%`（约 0.05px，仅为百分比两位小数量化），同页六个倾斜框最大偏差 `0.0052%`；背景子层均无二次旋转，文字与蓝框物理倾角最大误差 `0.044°`，`t0` 调试标注为 `-6.70984°`。完整 Node `850/850`、Python `86 passed, 1 skipped`、修改文件 ESLint 和构建通过；仓库行数门禁仅剩三个既有超长文件阻塞。

## 2026-08-09 - Codex���������Ŀ�֧�ֶ��������ԣ�

- �ֳ����������������װһ����չ��manifest �� `key` ʱ������װ����չ ID ��ͬ��Origin ��˲�ͬ���������Ŀ�ֻ����һ�� token ��ϣ + һ�� `extension_origin`��������ְ��������һ����ʹ�ã����µ�һ���������Ժ�ڶ���������޷�����ԣ���ʹ�õ����������Ҳ�Ḳ�ǵ�һ���������ƾ�ݡ�
- ���ƾ�ݸ�Ϊ����չ Origin �洢���飺���� `paired_extensions(extension_origin PRIMARY KEY, access_token_hash, paired_at)` ����`pair()` ֻ���¸� Origin �Լ��� token ��ϣ��`verify_access()` �� Origin �����ȶԹ�ϣ������ȫ�ֻ��⣻schema ���� v4���ɿ� `translation_meta` �� `access_token_hash`/`extension_origin` ���״γ�ʼ��ʱǨ�Ƶ��±���ɾ���ɼ���`has_saved_pairing` �Լ�����δǨ�Ƶľɿ⡣
- �����ӡ�������ֻ����һ�γɹ���ԡ��ſ�Ϊ��ͬһ Origin �ڱ�������ֻ������һ�Ρ�����һ�����������󣬵ڶ������������ͬһ������ԣ��� Origin ��������Լ����ڣ����������ǣ�������Եķ���������ʾ��Ϊ������루��������������������ԭ�������֤ʧЧʱ������ԣ�����ԭ�������֤ʧЧ����������������������ȡ���룬���֡���������ظ�ʹ�÷��� 403���ļ������塣
- ��չ������Ķ���ÿ����������б����Լ��� token �� Origin������ͷ��Я������ Origin������˰� Origin ����У�顣�����ع鸲�ǵڶ����������Գɹ�����������ֻƥ����� Origin���� Origin ���� 401��ͬ Origin �������ظ���Է��� 403���Լ��ɼ�Ǩ�ƺ����״̬������
- ���� Python ���� `19/19`������ Python `87 passed, 1 skipped`��Pylint `10.00/10`��Node `850/850`����չ������ `git diff --check` ͨ�����ֿ������Ž��Խ�ʣ�������������ļ�������

## 2026-08-09 - Codex���Ƴ��������Ŀ�����룬��Ϊ�޼�Ȩ����ֱ����

- �û�ȷ�ϲ�����Ҫ����룺���������ͬһδ�����չĿ¼��ͬһ Origin�������¡�����뵥��ʹ�á��롰��������������á��������ϻ��⣬��ÿ�η���������������뵼�·���������� 403�����ڶ��Ȿ�ؽ��̱���ֱ�Ӷ�ȡ `.local-data/translations.db` ���� SQLite����� token ��ʵ�����ؼ�ֵ���ޡ�
- ������Ƴ�ȫ����Ȩ��`/translations/pair` �˵㡢`TranslationPairPayload`��`TRANSLATION_PAIRING_CODE` ����������״̬��API �м����� Bearer+Origin У��ȫ��ɾ���������� `/translations/import` 10MiB ���ޱ����м����CORS ��ֻ���� `chrome-extension://[a-p]{32}` Origin����ͨ��ҳ��Ȼ�޷������ȡ��д�����Ŀ⡣schema �� v5��`paired_extensions` ���� `pairing.py`��`store.pair()`/`verify_access()` һ���Ƴ����ɿ��ʼ��ʱֱ�� `DROP TABLE IF EXISTS paired_extensions`������֤Ԫ���ݣ������������ݣ���
- ��չ�Ƴ���� UI ��ƾ���߼�����̨ `translation-service.js` ɾ�� token ����/�洢/У�顢`ensureLocalServiceToken`��`withLocalServiceAuth` �� 401 ��ƾ�ݷ�֧������������Я�� Authorization �� `X-Manga-Translator-Origin`������ҳ�뵯��ɾ�����������͡���ԡ���ť�����������״̬����loopback ��Ȩ̽�����̲��䣩�������ͬ����ȥ�صȸ�Ϊֱ�����󱾵ط���
- �ع���£�REST ���Ը�Ϊ��֤ `/translations/pair` ���� 404���޼�Ȩ����/������������ v4 ��Ա��������stream/debug ����ȥ����Բ��裻`translation_service_auth.test.mjs` ��дΪ���޼�Ȩֱ�� + �����Ѻ���ʾ + ���������ν�����顱��������֤��Python `85 passed, 1 skipped`��Pylint `10.00/10`��Node `845/845`����չ������ `git diff --check` ͨ�����ֿ������Ž��Խ�ʣ�������������ļ�������
- ʹ�÷�ʽ��Ϊ���������ط���������κ���ԣ���չ���κ��������ֱ�Ӷ�д���Ŀ⣬��������Ҳ�����ٳ��������ʧЧ���⡣ע�Ȿ�ط������ѽ���ͬ�����Ȿ�ؽ��̵� HTTP ���ã�������̱���ֱ�Ӷ�ȡ���ݿ��ļ�������δ����Ҫ���ϸ�߽���������豸����Ȩ��

## 2026-08-09 - Codex�����������������������ʾ��

- �ֳ�����ָ����ͣС˵������ʱС˵����ҳ��������״̬��ͬʱ���֡���������ʾ������������ϣ�.mt-floating-ball-wrap:hover .mt-{novel,webpage}-progress-panel��ֻҪָ������������������������һ����ʾ��
- ��ʾ������Ϊ���������.mt-floating-ball-group:has(.mt-floating-novel:hover, .mt-novel-progress-panel:hover) ֻ��ʾС˵��壬��ҳ��ͬ��ֻ��ʾ��ҳ��壬��ͣ������������հ״�����ʾ�κ���塣�������ֱ�� pointer-events: none ��Ϊ uto��ָ������Ƶ������ʱ��屣����ʾ�����Ķ�������ʱ�Բ�����ҳ��������
- �ع���£�overlay_style.test.mjs �������� :has ������ڡ��ɵ������� :hover �����ٳ��֡�������в�������Ϊ uto������ Node 845/845 ͨ������չ�����¹����� dist/extension/�����û�����չ����ҳ���¼��غ󸴺ˡ�

## 2026-08-09 - Codex���޶���������Ŀ�����󱣳ֹ���λ�ã�

- �ֳ�����ÿ���޸����ĺ���涼�����ͷ���������������������ڲ�����ɺ������ػ棺�޶���� `renderRevisionPanel` �����Ŀ� `refresh`/`render` ���� `replaceChildren()` ��չ����������ؽ����������ݸ߶�˲ʱ�������� `scrollTop` ���㣬�ػ���ɺ󲻻��Զ��ָ���
- �޶���������ػ�ǰ��¼ê�㣺��ʽ���뱻������Ƭ�� `item.id`�������޸�/���·���/AI �޶�/ɾ��/ѡ��/�̶�������������δ��ʽָ��ʱȡ��ǰ������������һ�ſɼ���Ƭ��ÿ�ſ�Ƭ���� `data-item-id`���ػ���ê�㿨Ƭ����ԭλ������ 8px �ϱ߾ࣩ����������״̬����Ӱ�졣
- ���Ŀ�ҳ `library.js` ͬ�����ػ�ǰ��¼������������һ�е� `recordKey`���нڵ�д�� `data-record-key`���ؽ���ָ����еĹ���λ�ã��������봥�����ػ�ͬ�����档
- �����ع飺`novel_revision_search.test.mjs` ����ê���¼/�ָ��߼��������������� `item.id`������ `translations_library_scroll.test.mjs` ������ê����ָ��߼������� Node `847/847` ͨ������չ�����¹����� `dist/extension/`�����û�����չ����ҳ���¼��غ󸴺ˡ�
