...
async function handleMessage(message, sender) {
  switch (message.type) {
    ...
    case "START_LOCAL_OCR":
      return handleStartLocalOcr();
    case "STOP_LOCAL_OCR":
      return handleStopLocalOcr();
    case "PING_LOCAL_OCR":
      return handlePingLocalOcr();
    default:
...
async function handleDiscoverTerms(message) {
  return enqueueTermDiscoveryMutation(async () => {
    ...
    try {
      ...
      // 服务端同步待确认候选
      try {
        const serverUrl = await isGlossaryServerMode();
        if (serverUrl && payload && payload.candidates) {
          for (const candidate of payload.candidates) {
            await callGlossaryApi(serverUrl, "/glossary/pending", {
              method: "POST",
              body: JSON.stringify({
    ...
    try {
      ...
      if (serverUrl) {
        for (const entry of requestedEntries) {
          await callGlossaryApi(serverUrl, "/glossary", {
            method: "PUT",
            body: JSON.stringify({
              source: entry.source,
              target: entry.target,
        ...
        for (const src of pendingSourcesToRemove) {
          await callGlossaryApi(serverUrl, "/glossary/pending/ignore", {
            method: "POST",
  ...
  return enqueueTermDiscoveryMutation(async () => {
    ...
    try {
      ...
      if (serverUrl && message.scope === "global") {
        await callGlossaryApi(serverUrl, "/glossary/pending/ignore", {
          method: "POST",
          body: JSON.stringify({ source: message.source })
        }).catch(() => {});
      }
    } catch (_) {}
...
async function handleRestoreIgnoredTerm(message) {
  return enqueueTermDiscoveryMutation(async () => {
    ...
    try {
      const serverUrl = await isGlossaryServerMode();
      if (serverUrl) {
        await callGlossaryApi(serverUrl, "/glossary/ignored/restore", {
          method: "POST",
          body: JSON.stringify({ source: message.source })
        }).catch(() => {});
      }
...
async function handleStartLocalOcr() {
  try {
    ...
    if (resp.ok) return { ok: true, status: "already_running" };
  } catch (_) {}
  return { ok: false, error: "请先手动运行 start_local_ocr_gpu.bat 启动 OCR 服务" };
}
...
async function callGlossaryApi(baseUrl, endpoint, options = {}) {
  const url = `${String(baseUrl || DEFAULT_LOCAL_OCR_BASE_URL).replace(/\/+$/, "")}${endpoint}`;
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), GLOSSARY_API_TIMEOUT_MS);
  try {
...
async function isGlossaryServerMode() {
  const stored = await storageGet([STORAGE_KEYS.glossaryStorage, STORAGE_KEYS.localOcrBaseUrl]);
  return stored[STORAGE_KEYS.glossaryStorage] === "server"
    ? stored[STORAGE_KEYS.localOcrBaseUrl] || DEFAULT_LOCAL_OCR_BASE_URL
    : "";
}
