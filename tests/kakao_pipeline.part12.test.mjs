...
test("seam-only complex evidence can request page artifacts after warm page OCR", async () => {
  ...
  const artifactRequests = harness.ocrMetas.filter(meta => meta.sourceType === "page" && meta.forceCleanedImageArtifact === true);
  assert.deepEqual(artifactRequests.map(meta => meta.pageIds[0]).sort(), ["page-a", "page-b"]);
  assert.deepEqual(artifactRequests.map(meta => ({
    pageId: meta.pageIds[0],
    masks: meta.cleanedMasks
  })), [{
    pageId: "page-a",
    masks: [{
...
test("late seam evidence reuses a cleaned artifact when the final blue mask is unchanged", async () => {
  ...
  options.pageObservations.b[0].visual = {
    ...options.pageObservations.b[0].visual,
    bgType: "none"
  };
